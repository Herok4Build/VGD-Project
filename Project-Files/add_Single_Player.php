...
session_start();
  ...
  <div class="collapse navbar-collapse " id="navbarSupportedContent">
    ...
    try{
	    ...
	    $single_player = $_POST["single_player"];
		$stmt = $DBH->prepare('INSERT INTO Video_Game_Inventory.Single_Player (Game_ID, Single_Player) VALUES (:videogame_id,:single_player)');
		$stmt->bindParam(":videogame_id",$videogame_id, PDO::PARAM_INT);
		$stmt->bindParam(":single_player",$single_player, PDO::PARAM_STR);
		$stmt->execute();
		$stmt_2 = $DBH->prepare('SELECT Single_Player.Game_ID, Game_Name, Single_Player FROM Single_Player, Games_list WHERE Single_Player.Game_ID = Games_List.Game_ID AND Single_Player.Game_ID = :videogame_id');
