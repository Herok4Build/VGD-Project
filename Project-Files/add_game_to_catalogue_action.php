<?php
//Thomas Johnson III
// 4-26-2019
// Database and Intelligent Systems
// Dr. Lin Chen
//Reference: https://www.tutorialspoint.com/php/php_mysql_login.htm
//Reference: https://www.tutorialrepublic.com/php-tutorial/php-mysql-login-system.php
//Dr. Lin Chen's notes
//Reference: https://stackoverflow.com/questions/4544051/sqlstate42000-syntax-error-or-access-violation-1064-you-have-an-error-in-you
//Reference: https://www.taniarascia.com/overview-of-sql-commands-and-pdo-operations/
//Reference:https://www.w3schools.com/php/php_mysql_insert.asp
//Reference: https://www.sitepoint.com/community/t/why-do-i-keep-getting-this-sqlstate-42000-error/6626/3
    session_start();
    
    if(!isset($_SESSION["loggedin"]) || $_SESSION["loggedin"] !== true)
    {
        header("location: ./video_game_welcome.html.php");
        exit;
    }
    include("phpconfiguration.php");// For accessing the database
    include("phpTemplate.php");//For PHP functions
    htmlHead("Add Game to Catalogue");
    pageBodyStart();
    
    echo '<nav class="navbar nav_border navbar-expand-lg navbar-light nav_background">
  <a class="navbar-brand" href="./view_game_catalogue.php">Your Catalogue</a>
  <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
    <span class="navbar-toggler-icon"></span>
  </button>

  <div class="collapse navbar-collapse " id="navbarSupportedContent">
    <ul class="navbar-nav mr-auto">
      <li class="nav-item">
        <a class="nav-link nav_text" href="./home.php">Home <span class="sr-only">(current)</span></a>
      </li>
      <li class="nav-item">
        <a class="nav-link nav_text" href="./view_Games_List.php">View All games</a>
      </li>
      <li class="nav-item">
        <a class="nav-link nav_text" href="./view_Game_Genres.php">View Game Genres</a>
      </li>
      <li class="nav-item">
        <a class="nav-link nav_text" href="./view_Game_Systems.php">View Game Systems</a>
      </li>
      <li class="nav-item">
      <a class="nav-link nav_text" href="./view_Contact_Info.php">Contact Info</a>
      </li>
      <li class="nav-item">
        <a class="nav-link disabled" href="video_game_welcome.html">Sign In</a>
      </li>
      <li class="nav-item">
      <a class="nav-link nav_text" href="./logout_action.php">Logout</a>
      </li>
    </ul>
  </div>
    </nav>';
    
    try{
	    $current_user = $_SESSION["username"];
	    $videogame_id = $_POST["videogame_id"];
	    $date_added = $_POST["date_added"];
	    $user_notes = $_POST["user_notes"];
	    $users_rating = $_POST["rating"];
	    $videogame_condition = $_POST["videogame_condition"];
		$stmt = $DBH->prepare('INSERT INTO Video_Game_Inventory.Games (Username,Game_ID,Date_Added,Notes,Rating,Game_Condition) VALUES (:current_user, :videogame_id, :date_added, :user_notes, :rating, :videogame_condition)');
		$stmt->bindParam(":current_user",$current_user, PDO::PARAM_STR);
		$stmt->bindParam(":videogame_id",$videogame_id, PDO::PARAM_INT);
		$stmt->bindParam(":date_added",$date_added, PDO::PARAM_STR);
		$stmt->bindParam(":user_notes",$user_notes, PDO::PARAM_STR);
		$stmt->bindParam(":rating",$users_rating, PDO::PARAM_INT);
		$stmt->bindParam(":videogame_condition",$videogame_condition, PDO::PARAM_STR);
		$stmt->execute();
		$stmt_2 = $DBH->prepare('SELECT Username, Games.Game_ID, Game_Name, Date_Added, Notes, Rating, Game_Condition FROM Games, Games_List WHERE Username = :current_user AND Games.Game_ID = Games_List.Game_ID');
		$stmt_2->bindParam(":current_user",$current_user, PDO::PARAM_STR);
		$stmt_2->execute();
		echo "<br>";
		echo '<table style="margin-right:auto;margin-left:auto;" bgcolor="#F8F8F8" cellpadding="5px">';
		echo "<tr><th>Game ID</th><th>Game Name</th><th>Date Added</th><th>Notes</th><th>Rating</th><th>Game Condition</th></tr>";
		while($row = $stmt_2->fetch())
		{
		echo "<tr><td>".$row['Game_ID']."</td> <td>".$row['Game_Name']."</td> <td>".$row['Date_Added']."</td> <td>".$row['Notes']."</td><td>".$row['Rating']."</td><td>".$row['Game_Condition']."</td></tr>";
		}
		echo "</table>";
		$DBH = NULL;
	}
	catch(PDOException $e){
		echo $e->getMessage();
	}
	pageBodyEnd();
?>