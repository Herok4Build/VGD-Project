...
htmlHead("Add Game as Multiplayer");
session_start();
  ...
  <div class="collapse navbar-collapse " id="navbarSupportedContent">
    ...
    try{
		...
		$stmt = $DBH->prepare('INSERT INTO Video_Game_Inventory.Multiplayer (Game_ID, Multiplayer) VALUES (:videogame_id,:multiplayer)');
		$stmt->bindParam(":videogame_id",$videogame_id, PDO::PARAM_INT);
		$stmt->bindParam(":multiplayer",$multiplayer, PDO::PARAM_STR);
		$stmt->execute();
		$stmt_2 = $DBH->prepare('SELECT Multiplayer.Game_ID, Game_Name, Multiplayer FROM MultiPlayer, Games_list WHERE Multiplayer.Game_ID = Games_List.Game_ID AND Multiplayer.Game_ID = :videogame_id');
		...
		echo "<tr><th>Game ID</th><th>Game Name</th><th>Multiplayer</th></tr>";
		while($row = $stmt_2->fetch())
		{
		echo "<tr><td>".$row['Game_ID']."</td> <td>".$row['Game_Name']."</td> <td>".$row['Multiplayer']."</td></tr>";
		}
		echo "</table>";
		$DBH = NULL;
		}
